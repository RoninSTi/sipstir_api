...
class User extends Model {
  ...
  static async getSingle({ client, id, redis }) {
    ...
    const allTimeLeaderboardPosition = await redis.zrevrank(
      "leaderboard",
      user.id
    );
    ...
    const followerIds = streamFollowers
      .map(({ feed_id }) => parseUserId(feed_id))
      .filter((followerId) => followerId !== id);
    ...
    const followingIds = streamFollowing.map((obj) =>
      parseUserId(obj.target_id)
    );
